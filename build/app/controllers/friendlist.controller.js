...
export default {
    getPendingRequestSent: async (req, res) => {
    ...
    getAcceptedFriends: async (req, res) => {
        const userId = checkParams(req.params.id);
        const status = 'accepted';
        const friends = await Friends.find(userId, status);
        res.status(200).json({ message: 'Friends retrieved successfully', data: friends });
    },
    getPendingRequestReceived: async (req, res) => {
        const userId = checkParams(req.params.id);
    ...
    acceptFriendRequest: async (req, res) => {
        ...
        try {
            ...
            if (error instanceof NotFoundError)
                return res.status(200).json({ error: 'No pending friend request found' });
        }
    ...
    rejectFriendRequest: async (req, res) => {
        ...
        try {
            ...
            if (error instanceof NotFoundError)
                return res.status(200).json({ message: 'No pending friend request found' });
        }
