...
export default {
    find: async (event_id, user_id) => {
        try {
            ...
            if (!userOnEvent)
                throw new NotFoundError('No user on event found');
            // @ts-ignore
    ...
    createMany: async (event_id, userIds) => {
        try {
    ...
    update: async (user_id, event_id, status, team) => {
        try {
